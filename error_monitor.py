...
WATCH_TARGETS: Final[dict[str, Path]] = {
    "Web" : (PROJECT_ROOT / "logs" / "web.log").resolve(),
    "Admin" : (PROJECT_ROOT / "logs" / "admin.log").resolve(),
    "Driver": (PROJECT_ROOT / "logs" / "driver.log").resolve(),
}

# 하위 호환: cleanup에서 사용하는 단일 경로 (백엔드 기준)
WATCH_LOG_PATH: Final[Path] = WATCH_TARGETS["Web"]
...
SOURCE_FILES: Final[list[Path]] = [
    # Monorepo Common Packages
    PROJECT_ROOT / "packages" / "lib" / "src" / "index.ts",
    PROJECT_ROOT / "packages" / "ui" / "src" / "index.ts",
    
    # apps/web
    PROJECT_ROOT / "apps" / "web" / "src" / "app" / "layout.tsx",
    PROJECT_ROOT / "apps" / "web" / "src" / "app" / "page.tsx",
    
    # apps/admin
    PROJECT_ROOT / "apps" / "admin" / "src" / "app" / "layout.tsx",
    PROJECT_ROOT / "apps" / "admin" / "src" / "app" / "page.tsx",
    
    # apps/driver
    PROJECT_ROOT / "apps" / "driver" / "src" / "app" / "layout.tsx",
    PROJECT_ROOT / "apps" / "driver" / "src" / "app" / "page.tsx",
]
...
_ERROR_PATTERN: re.Pattern[str] = re.compile(
    r"\b("
    # Python / Backend
    r"ERROR|CRITICAL|Traceback|ModuleNotFoundError|AttributeError|ValueError|TypeError"
    r"|"
    # JavaScript / TypeScript / Next.js (빌드/런타임 에러)
    r"FAILED|Unhandled|Uncaught|SyntaxError|ReferenceError|RangeError"
    r"|"
    # Chromium / Tauri / Rust
    r"panicked|RUST_BACKTRACE|error\[|ENOENT|ECONNREFUSED"
    r")\b",
    re.IGNORECASE,
)

# 에러 패턴에 걸려도 실제로는 무시해야 할 노이즈 패턴
_IGNORE_PATTERN: re.Pattern[str] = re.compile(
    r"("
    # Next.js 이미지 최적화 upstream 404 (외부 이미지 소스 이슈, 코드 에러 아님)
    r"upstream image response failed"
    r"|"
    # Next.js config 변경 감지 후 자동 재시작 (정상 동작)
    r"Found a change in next\.config"
    r"|"
    # 이미지 404 응답 (외부 CDN 이슈)
    r"/_next/image.*404"
    r")",
    re.IGNORECASE,
)
...
def run_monitor(clear_at_start: bool = False) -> None:
    ...
    for label, log_path in WATCH_TARGETS.items():
        ...
        try:
            ...
            print(f"[!] [{label}] 파일이 사용 중이거나 잠겨있어 초기화 건너뜀: {exc}")
    ...
    while True:
        for label, log_path in WATCH_TARGETS.items():
            ...
            if current_mtime != last_mtimes[label]:
                ...
                if _ERROR_PATTERN.search(log_tail) and not _IGNORE_PATTERN.search(log_tail):
                    now = time.monotonic()
        ...
        for label, log_path in WATCH_TARGETS.items():
            if log_path.exists():
                try:
                    ...
                    print(f"[!] [{label}] 로그 초기화 건너뜀 (사용 중): {exc}")
